                                ...
                                <tr class="bg-white border border-slate-50 shadow-sm rounded-2xl hover:bg-slate-50/50 transition-all">
                                    <td class="px-6 py-5 rounded-l-3xl">
                                        <div class="flex items-center">
                                            <div class="h-12 w-12 rounded-2xl {{ strtolower($user->status ?? '') === 'pending' ? 'bg-amber-50 text-amber-600' : 'bg-indigo-50 text-indigo-600' }} flex items-center justify-center font-black">
                                        ...
                                        <div class="flex justify-end items-center gap-2">
                                            @if(!$user->is_approved)
                                                <form action="{{ route('admin.admin.usuarios.approve', $user->id) }}" method="POST">
                                                    @csrf @method('PATCH')
                                                    <button type="submit" class="px-3 py-2 bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white rounded-xl transition-all font-bold text-xs" title="Aprobar">Aprobar</button>
                                                </form>
                                                <form action="{{ route('admin.admin.usuarios.decline', $user->id) }}" method="POST" onsubmit="return confirm('¿Rechazar usuario?')">
                                                    @csrf @method('DELETE')
                                                    <button class="px-3 py-2 bg-rose-50 text-rose-600 hover:bg-rose-600 hover:text-white rounded-xl transition-all font-bold text-xs" title="Declinar">X</button>
                                                </form>
                                            @endif
